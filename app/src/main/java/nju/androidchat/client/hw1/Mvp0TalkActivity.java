...
public class Mvp0TalkActivity extends AppCompatActivity implements Mvp0Contract.View, TextView.OnEditorActionListener, OnRecallMessageRequested {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.presenter = new PictureTalkPresenter(mvp0TalkModel, this, new ArrayList<>());
        ...
        runOnUiThread(() -> {
                    ...
                    for (ClientMessage message : messages) {
                        ...
                        if (message.getSenderUsername().equals(this.presenter.getUsername())) {
                            if (message instanceof ClientPictureMessage){
                                content.addView(new ItemTextSend(extractPic(message),this, text, message.getMessageId(), this));
                            }
                            else{
                            ...
                            if (message instanceof  ClientPictureMessage){
                                content.addView(new ItemTextReceive(extractPic(message),this, text, message.getMessageId()));
                            }
                            else{
    ...
    private String extractPic(ClientMessage message){
